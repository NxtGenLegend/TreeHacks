...
export function Chat({ courseId, lectureId }: ChatProps) {
  ...
  const getVLMresults = async (vlm_input: string) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/query_vlm?query="+vlm_input, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/json",
  ...
  const getVLMresults = async (vlm_input: string) => {
    try {
      const response = await fetch("http://localhost:8000/query_vlm", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Accept": "*",
        },
        body: JSON.stringify({ query: vlm_input }),
      });
      ...
      console.error("Failed to search VLM:", error.message);
    }
    return "";
  };
  ...
  const handleSend = async () => {
    ...
    // console.log("VLM Image:", vlmImage);
